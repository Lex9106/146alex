...
function setup(eim, leaderid) {
    ...
    var eim = em.newInstance("VonBonEasy");
		var map = eim.setInstanceMap(105200110);
    map.resetFully();
   // eim.setInstanceMap(105200810).resetFully();
	//    eim.setInstanceMap(401060200).resetFully();
	var mob0 = em.getMonster(8910101);
...
//	mob0.setOvverideStats.exp(10000);
	map.spawnMonsterOnGroundBelow(mob0, new java.awt.Point(-195, 455));
    eim.startEventTimer(1* 1800000); //now changed to 1 hour 15 mins
    return eim;
}

function playerEntry(eim, player) {
    var map = eim.getMapFactory().getMap(105200110);
...
function end(eim) {
    if (eim.disposeIfPlayerBelow(100, 105200110)) {
	em.setProperty("state", "0");
		em.setProperty("leader", "true");
