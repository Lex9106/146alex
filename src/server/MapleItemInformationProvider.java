...
import static handling.channel.handler.InventoryHandler.Reveal;
...
public class MapleItemInformationProvider {
    public static transient int PREMIUM = 0;
    ...
    public final Map<Integer, ItemInformation> dataCache = new HashMap<>();
    ...
    protected final Map<Integer, Integer> setLevels = new HashMap<>();
    ...
    public void runEtc() {
        ...
        for (MapleData dat : setsData) {
            ...
            for (MapleData level : dat.getChildByPath("Effect")) {
                ...
                itez.activeSkill = MapleDataTool.getIntConvert("activeSkill/0/id", level, 0);
                itemz.items.put(Integer.parseInt(level.getName()), itez);
            }
            setItems.put(itemz.setItemID, itemz);
            //System.out.println(itemz.setItemID + " Set item Id has been loaded.");
        }
    ...
    public final boolean canEquipDemon(final Map<String, Integer> stats, final int itemid, final int level, final int job, final int fame, final int luk, final int int_, int supremacy, byte reqLevel) {
        if (level + supremacy >= 0xFF) {
            supremacy = 0xFF - level;
        }
        if ((level + supremacy) >= (reqLevel != 0 ? reqLevel : stats.containsKey("reqLevel") ? stats.get("reqLevel") : 0) && luk >= (stats.containsKey("reqLUK") ? stats.get("reqLUK") : 0) && int_ >= (stats.containsKey("reqINT") ? stats.get("reqINT") : 0)) {
            final Integer fameReq = stats.get("reqPOP");
            return fameReq == null || fame >= fameReq;
        }
        return false;
    ...
    public final int getReqJob(final int itemId) {
        ...
        //FileoutputUtil.log("ReqJob.txt", "" + getName(itemId) + ",//" + getEquipStats(itemId).get("reqJob") + "\r\n");
    ...
    public final Integer getSetItemID(final int itemId) {
        if (getEquipStats(itemId) == null && !GameConstants.isPet(itemId) || !getEquipStats(itemId).containsKey("setItemID") && !GameConstants.isPet(itemId)) {
            return 0;
        }
    ...
    public final Integer getSetLevel(final int setLevel) {
        return setLevels.get(setLevel);
    }
    ...
    public final Item scrollEquipWithId(final Item equip, final Item scrollId, final boolean ws, final MapleCharacter chr, final int vegas) {
        //if (equip.getType() == 1) { // See Item.java
            ...
            if (ItemFlag.LUCKY_DAY.check(equip.getFlag()) && !GameConstants.isPotentialScroll(scrollId.getItemId()) && !GameConstants.isCashSpecialScroll(scrollId.getItemId()) && !GameConstants.isEquipScroll(scrollId.getItemId()) && !GameConstants.isSpecialScroll(scrollId.getItemId()) && !GameConstants.isInnocenceScroll(scrollId.getItemId())) {
                equip.setFlag((short) (equip.getFlag() - ItemFlag.LUCKY_DAY.getValue()));
            }
            if (ItemFlag.SLOTS_PROTECT.check(equip.getFlag()) && !GameConstants.isPotentialScroll(scrollId.getItemId()) && !GameConstants.isCashSpecialScroll(scrollId.getItemId()) && !GameConstants.isEquipScroll(scrollId.getItemId()) && !GameConstants.isSpecialScroll(scrollId.getItemId()) && !GameConstants.isInnocenceScroll(scrollId.getItemId())) {
                equip.setFlag((short) (equip.getFlag() - ItemFlag.SLOTS_PROTECT.getValue()));
            }
            if (chr.isSuperGM()) {
            chr.dropMessage(5,"Success rate is: "+succ+"%");
            }
            if (GameConstants.isPotentialScroll(scrollId.getItemId()) || GameConstants.isEquipScroll(scrollId.getItemId()) || GameConstants.isInnocenceScroll(scrollId.getItemId()) ||  GameConstants.isCashSpecialScroll(scrollId.getItemId())  || GameConstants.isSpecialScroll(scrollId.getItemId()) || Randomizer.nextInt(100) <= success) {
               PREMIUM = 1;//Success.
                switch (scrollId.getItemId()) {
                    ...
                    case 2046685: {
                    nEquip.setWatk((short) (nEquip.getWatk()+ 1 + (int)(Math.random() * 1)));
                    }
                    case 2046686: {
                    nEquip.setMatk((short) (nEquip.getMatk()+ 1 + (int)(Math.random() * 1)));
                    }
                    case 2046829: {
                    nEquip.setWatk((short) (nEquip.getWatk()+ 2 + (int)(Math.random() * 2)));
                    }
                    case 2046830: {
                    nEquip.setMatk((short) (nEquip.getMatk()+ 2 + (int)(Math.random() * 2)));
                    }
                    case 2048082: {
                    nEquip.setWatk((short) (nEquip.getWatk()+ 2 + (int)(Math.random() * 2)));
                    }
                    ...
                    case 2048094: {
                    nEquip.setWatk((short) (nEquip.getWatk()+ 4 + (int)(Math.random() * 1)));
                    }
                    case 2048095: {
                    nEquip.setMatk((short) (nEquip.getMatk()+ 4 + (int)(Math.random() * 1)));
                    }
                    case 5064000:          
                    case 2531004:
                    case 5064003: {
    ...
    if(Randomizer.nextInt(100) <= succ){
        ...
        nEquip.setTotalDamage(template.getTotalDamage()); 
        nEquip.setAllStat(template.getAllStat()); 
        nEquip.setBossDamage(template.getBossDamage()); 
        nEquip.setIgnorePDR(template.getIgnorePDR()); 
        ...
        randomizeStatsInnocence(nEquip, chr, c);
        chr.getMap().broadcastMessage(chr, CField.getScrollEffect(chr.getId(), Equip.ScrollResult.SUCCESS, false, equip.getItemId(), scrollId.getItemId()), vegas == 0);
    } else {
        chr.getMap().broadcastMessage(chr, CField.getScrollEffect(chr.getId(), Equip.ScrollResult.FAIL, false, equip.getItemId(), scrollId.getItemId()), vegas == 0);
    }
                        }
                        ...
                        else if(GameConstants.iicsog(scrollId.getItemId())) {//Incredible chaos scroll of goodnes
                            final int z = GameConstants.geticsogNumber(scrollId.getItemId());
                            if (nEquip.getStr() > 0) {
                                nEquip.setStr((short) (nEquip.getStr() + Randomizer.nextInt(z)) );
                            }
                            if (nEquip.getDex() > 0) {
                        ...
                        else if (GameConstants.isChaosScroll(scrollId.getItemId()) && scrollId.getItemId() != 2049121) {
                            final int z = GameConstants.getChaosNumber(scrollId.getItemId());
                            if (nEquip.getStr() > 0) {
                        ...
                        if (GameConstants.isSpecialScroll(scrollId.getItemId())) {
                            ...
                            if (nEquip.getState() <= 17 && (scrollId.getItemId() / 100 == 20497) && (scrollId.getItemId() / 10 != 204975)) {
                                final int chanc = (scrollId.getItemId() == 2049701 ? 80 : 100) + added; // 2049701
                                if (Randomizer.nextInt(100) > chanc) {
                                }
                                else {
                                 nEquip.renewPotential(2, 0, (short) 0);
                                }
                            }
                            if (nEquip.getState() <= 18 && (scrollId.getItemId() / 10 == 204975)) {
                                final int chanc = (scrollId.getItemId() == 2049701 ? 80 : 100) + added; // 2049701
                                if (Randomizer.nextInt(100) > chanc) {
                                }
                                else {
                                 nEquip.renewUniquePotential(2, 0, (short) 0);
                                }
                            }
                ...
                if (!GameConstants.isCleanSlate(scrollId.getItemId()) && scrollId.getItemId() != 2049121 && !GameConstants.isCashSpecialScroll(scrollId.getItemId()) && !GameConstants.isSpecialScroll(scrollId.getItemId()) && !GameConstants.isInnocenceScroll(scrollId.getItemId()) && !GameConstants.isEquipScroll(scrollId.getItemId()) && !GameConstants.isPotentialScroll(scrollId.getItemId())) {
                ...
                if (!ws && !GameConstants.isCleanSlate(scrollId.getItemId()) && scrollId.getItemId() != 2049121 && !GameConstants.isCashSpecialScroll(scrollId.getItemId()) && !GameConstants.isSpecialScroll(scrollId.getItemId()) && !GameConstants.isInnocenceScroll(scrollId.getItemId()) && !GameConstants.isEquipScroll(scrollId.getItemId()) && !GameConstants.isPotentialScroll(scrollId.getItemId())) {
                    if (GameConstants.isAzwanScroll(scrollId.getItemId())) {
                        nEquip.setUpgradeSlots((byte) (nEquip.getUpgradeSlots() - stats.get("tuc")));
                    } else {
                        nEquip.setUpgradeSlots((byte) (nEquip.getUpgradeSlots() - 1));
     ...
     public List<Integer> getItemIdFromName(String name, MapleCharacter chr) {   
            MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();         
            List <Integer> itemReturn = new LinkedList <Integer> ();
           ...
           int reqJob = ii.getReqJob(itemPair.getLeft());
           int reqLevel = ii.getReqLevel(itemPair.getLeft());
               if (itemPair.getRight().toLowerCase().contains(name.toLowerCase()) && itemPair.getLeft() > 0) {
                   if (name.isEmpty()) {
                   if (GameConstants.isWarriorJob(chr.getJob())) {
                   if (reqJob == 1 && reqLevel >= 75 || reqJob == 13 && reqLevel >= 75) {
                   itemReturn.add(itemPair.getLeft());
                   }
                   }
                   else if (GameConstants.isMageJob(chr.getJob())) {
                   if (reqJob == 2 && reqLevel >= 75) {
                   itemReturn.add(itemPair.getLeft());
                   }
                   }
                   else if (GameConstants.isArcherJob(chr.getJob())) {
                   if (reqJob == 4 && reqLevel >= 75) {
                   itemReturn.add(itemPair.getLeft());
                   }
                   }
                   else if (GameConstants.isThiefJob(chr.getJob())) {
                   if (reqJob == 8 && reqLevel >= 75 || reqJob == 9 && reqLevel >= 75) {
                   itemReturn.add(itemPair.getLeft());
                   }
                   }
                   else if (GameConstants.isPirateJob(chr.getJob())) {
                   if (reqJob == 16 && reqLevel >= 75 || reqJob == 24 && reqLevel >= 75) {
                   itemReturn.add(itemPair.getLeft());
                   }
                   }
               }
    ...
    protected final short getRandStatNoNegative(final short defaultValue, final int maxRange) {
        if (defaultValue == 0) {
            return 0;
        }
        // vary no more than ceil of 10% of stat
        final int lMaxRange = (int) Math.min(Math.ceil(defaultValue * 0.1), maxRange);
        return (short) ((defaultValue + (Randomizer.nextInt(5))) + Randomizer.nextInt(lMaxRange * 2 + 1));
    }
    ...
    public final Equip randomizeStats(final Equip equip, MapleCharacter chr, final MapleClient c) {
        ...
        equip.setWdef(getRandStat(equip.getWdef(), 5));
        equip.setMdef(getRandStat(equip.getMdef(), 5));
        equip.setHp(getRandStat(equip.getHp(), 5));
        equip.setMp(getRandStat(equip.getMp(), 5));
        ...
        c.getSession().write(EffectPacket.TutInstructionalBalloon("Effect/Direction.img/effect/cygnus/back1/0"));
        c.getSession().write(EffectPacket.TutInstructionalBalloon("Effect/Direction.img/effect/cygnus/final7/0"));
        chr.dropMessage(-1, msg + " has been blessed with the power of Cygnus.");
        c.getSession().write(CField.playSound("PetPerfume/Use")); 
        equip.setStr(getRandStatNoNegative(equip.getStr(), 5));
        equip.setDex(getRandStatNoNegative(equip.getDex(), 5));
        equip.setInt(getRandStatNoNegative(equip.getInt(), 5));
        equip.setLuk(getRandStatNoNegative(equip.getLuk(), 5));
        equip.setMatk(getRandStatNoNegative(equip.getMatk(), 5));
        equip.setWatk(getRandStatNoNegative(equip.getWatk(), 5));
        equip.setAcc(getRandStatNoNegative(equip.getAcc(), 5));
        equip.setAvoid(getRandStatNoNegative(equip.getAvoid(), 5));
        equip.setJump(getRandStatNoNegative(equip.getJump(), 5));
        equip.setHands(getRandStatNoNegative(equip.getHands(), 5));
        equip.setSpeed(getRandStatNoNegative(equip.getSpeed(), 5));
        equip.setWdef(getRandStatNoNegative(equip.getWdef(), 5));
        equip.setMdef(getRandStatNoNegative(equip.getMdef(), 5));
        equip.setHp(getRandStatNoNegative(equip.getHp(), 5));
        equip.setMp(getRandStatNoNegative(equip.getMp(), 5));
        equip.setBossDamage(getRandStatNoNegative(equip.getBossDamage(), 5));
        equip.setTotalDamage(getRandStatNoNegative(equip.getTotalDamage(), 5));
        equip.setIgnorePDR(getRandStatNoNegative(equip.getIgnorePDR(), 5));
        if (reqLevel > 1 && reqLevel < 70 && !GameConstants.isWeapon(equip.getItemId())) {
        equip.setAllStat((short) Randomizer.nextInt(1));    
        }
        if (reqLevel > 69 && reqLevel < 120 && !GameConstants.isWeapon(equip.getItemId())) {
        equip.setAllStat((short) ((short) 1 + Randomizer.nextInt(2)));    
        }
        if (reqLevel > 120 && !GameConstants.isWeapon(equip.getItemId())) {
        equip.setAllStat((short) ((short) 3 + Randomizer.nextInt(2)));    
        }
        if (GameConstants.isWeapon(equip.getItemId()) && equip.getBossDamage() == 0 && equip.getIgnorePDR()== 0 && reqLevel > 69 && reqLevel < 120) {
        equip.setBossDamage((short) Randomizer.nextInt(3));
        equip.setTotalDamage((short) Randomizer.nextInt(3));
        }
        if (GameConstants.isWeapon(equip.getItemId()) && equip.getBossDamage() == 0 && equip.getIgnorePDR()== 0 && reqLevel > 119 && reqLevel < 150) {
        equip.setBossDamage((short) ((short) 3 + Randomizer.nextInt(2)));
        equip.setIgnorePDR((short) ((short) 3 + Randomizer.nextInt(2)));
        equip.setTotalDamage((short) ((short) 3 + Randomizer.nextInt(2)));
        }
        if (GameConstants.isWeapon(equip.getItemId()) && equip.getBossDamage() == 0 && equip.getIgnorePDR()== 0 && reqLevel > 149 && reqLevel < 251) {
        equip.setBossDamage((short) ((short) 5 + Randomizer.nextInt(5)));
        equip.setIgnorePDR((short) ((short) 5 + Randomizer.nextInt(5)));
        equip.setTotalDamage((short) ((short) 5 + Randomizer.nextInt(5)));
        }
        }
        return equip;      
    }
    
    public final Equip randomizeStatsWheel(final Equip equip, MapleCharacter chr, final MapleClient c) {
    ...
    public final Equip randomizeStatsInnocence(final Equip equip, MapleCharacter chr, final MapleClient c) {
    ...
    public final Equip randomizeStats_Above(final Equip equip) {
        ...
        equip.setWdef(getRandStatAbove(equip.getWdef(), 5));
        equip.setMdef(getRandStatAbove(equip.getMdef(), 5));
        equip.setHp(getRandStatAbove(equip.getHp(), 5));
        equip.setMp(getRandStatAbove(equip.getMp(), 5));
        return equip;
    }
    ...
    public void finalizeEquipData(ItemInformation item) {
        ...
        if (item.equipStats.size() > 0) {
            for (Entry<String, Integer> stat : item.equipStats.entrySet()) {
                ...
                switch (key) {
                    ...
                    case "damR":
                        ...
                        break;
                    case "allStat":
                        item.eq.setAllStat(stat.getValue().shortValue());
                        break;
                }
            }
            ...
            /*final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
                 final String msg = ii.getName(item.eq.getItemId());
                 if (GameConstants.isOverall(item.eq.getItemId()) && !ii.isCash(item.eq.getItemId()) && !ii.isOnlyTradeBlock(itemId)) {
                 FileoutputUtil.log("Equip.txt", "" + item.eq.getItemId() + ",//" + msg + "\r\n");
                 }*/
        }
    }
