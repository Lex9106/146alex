...
public final class MapleMap {
    ...
    */
            ...
            if (Randomizer.nextInt(999999) < (int) (de.chance * chServerrate * chr.getDropMod() * chr.getStat().dropBuff / 100.0 * (showdown / 100.0))) {
                ...
                if (de.itemId == 0) { // meso
                    ...
                    if (GameConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
                        idrop = ii.randomizeStats((Equip) ii.getEquipById(de.itemId), chr, chr.getClient());
                    } else {
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill) {
        if (chr.getStat().hpRecoverPropOnKill > 0) {
            if (Randomizer.nextInt(100) <= chr.getStat().hpRecoverPropOnKill) {//i think its out of 100, anyway
                if (chr.getStat().hpRecoverOnKill > 0) {
                    chr.healHP(chr.getStat().hpRecoverOnKill * chr.getStat().getCurrentMaxHp() / 100);
                }
            }
        }
        MobKillNumber ++;
        ...
        Skill bx;
            ...
            MapleQuestStatus marr = chr.getQuestNAdd(MapleQuest.getInstance(GameConstants.AFFINITY_HEARTIV));
                    if (marr.getCustomData() == null) {
                        marr.setCustomData("0");
                    }
            long lastTime = Long.parseLong(marr.getCustomData());
        if (chr.getStat().MonsterKills > 0) {
        if (chr.getStat().MonsterKills == MobKillNumber) {
        chr.getClient().getSession().write(AngelicPacket.unlockSkill());
        chr.getClient().getSession().write(AngelicPacket.showRechargeEffect());
        MobKillNumber = 0;//Reset When Succed
        if (!(lastTime + (5000) > System.currentTimeMillis()) && Randomizer.nextInt(100) <= 50 && bof > 0) {
        ...
        if (chr.getStat().mpRecoverPropOnKill > 0) {
            if (Randomizer.nextInt(100) <= chr.getStat().mpRecoverPropOnKill) {//i think its out of 100, anyway
                if (chr.getStat().mpRecoverOnKill > 0) {
                    chr.healMP(chr.getStat().mpRecoverOnKill * chr.getStat().getCurrentMaxMp(chr.getJob())/ 100);
                }
            }
        }
         else if (monster.getId() == 9390602) {//Gollux
            if (chr.getQuestStatus(45) == 2 && chr.getQuestStatus(46) == 2 && chr.getQuestStatus(47) == 2) {
            chr.finishAchievement(48);
            }
            if (Randomizer.nextInt(100) <= 10) {
            chr.addReward(1, 2615001, 0, 0, 0, "Reward for defeating Gollux and all it's parts!");
            }
            else if (Randomizer.nextInt(100) <= 10) {
            chr.addReward(1, 2615000, 0, 0, 0, "Reward for defeating Gollux and all it's parts!");
            }
            else if (Randomizer.nextInt(100) <= 5) {
            chr.addReward(1, 2003048, 0, 0, 0, "Reward for defeating Gollux and all it's parts!");
            }
            else if (Randomizer.nextInt(100) <= 5) {
            chr.addReward(1, 2003049, 0, 0, 0, "Reward for defeating Gollux and all it's parts!");
            }
            else {
            chr.addReward(1, 4310097, 0, 0, 0, "Reward for defeating Gollux and all it's parts!");    
            }
            chr.updateReward();
        }
        ...
        short DifPlus = (short) (chr.getLevel() - monster.getStats().getLevel());
         ...
         if (chr.getLevel() >= monster.getStats().getLevel()) {
         CharDifference = 1;    
         }
         int MobDifference = 0;
         if (monster.getStats().getLevel() >= chr.getLevel()) {
         MobDifference = 1;    
         }
        if (monster.getStats().isElite()) {
        MapleReactor reactor = new MapleReactor(MapleReactorFactory.getReactor(9802000), 9802000);   
        reactor.setState((byte) 1);
        spawnReactorOnGroundBelow(reactor, new Point(monster.getTruePosition().x, monster.getTruePosition().y - 40));   
        }
        InterServerHandler.EliteBossTime = 300000;
        if (InterServerHandler.now > 0 && Randomizer.nextInt(250) <= 1) {
         if (chr.getLevel() >= 100 && CharDifference >= 1 && DifPlus >= 0 && DifPlus <= 25 && GameConstants.isWarriorJob(chr.getJob()) || MobDifference > 0 && DifMin >= 0 && DifMin <= 25 && GameConstants.isWarriorJob(chr.getJob())) {
         chr.dropMessage(-1, "You can feel an evil energy comming, watch yourself!");
         chr.getClient().getSession().write(CField.playSound("ShamanBTTuto/aka"));
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8220022), Pos);
                    try {
                        Thread.sleep(5000);
        ...
        InterServerHandler.now = 0;
        }
         if (chr.getLevel() >= 100 && CharDifference >= 1 && DifPlus >= 0 && DifPlus <= 25 && GameConstants.isMageJob(chr.getJob()) || MobDifference > 0 && DifMin >= 0 && DifMin <= 25 && GameConstants.isMageJob(chr.getJob())) {
         chr.dropMessage(-1, "You can feel an evil energy comming, watch yourself!");
         chr.getClient().getSession().write(CField.playSound("ShamanBTTuto/aka"));
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8220023), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        InterServerHandler.now = 0;
         ...
         if (chr.getLevel() >= 100 && CharDifference >= 1 && DifPlus >= 0 && DifPlus <= 25 && GameConstants.isArcherJob(chr.getJob()) || MobDifference > 0 && DifMin >= 0 && DifMin <= 25 && GameConstants.isArcherJob(chr.getJob())) {
         chr.dropMessage(-1, "You can feel an evil energy comming, watch yourself!");
         chr.getClient().getSession().write(CField.playSound("ShamanBTTuto/aka"));
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8220024), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        InterServerHandler.now = 0;
         ...
         if (chr.getLevel() >= 100 && CharDifference >= 1 && DifPlus >= 0 && DifPlus <= 25 && GameConstants.isThiefJob(chr.getJob()) || MobDifference > 0 && DifMin >= 0 && DifMin <= 25 && GameConstants.isThiefJob(chr.getJob())) {
         chr.dropMessage(-1, "You can feel an evil energy comming, watch yourself!");
         chr.getClient().getSession().write(CField.playSound("ShamanBTTuto/aka"));
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8220025), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        InterServerHandler.now = 0;
         ...
         if (chr.getLevel() >= 100 && CharDifference >= 1 && DifPlus >= 0 && DifPlus <= 25 && GameConstants.isPirateJob(chr.getJob()) || MobDifference > 0 && DifMin >= 0 && DifMin <= 25 && GameConstants.isPirate(chr.getJob())) {
         chr.dropMessage(-1, "You can feel an evil energy comming, watch yourself!");
         chr.getClient().getSession().write(CField.playSound("ShamanBTTuto/aka"));
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8220026), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (InterServerHandler.now > 0 && Randomizer.nextInt(100) <= 1) {
        if (monster.getId() == 2100102) {
         if (CharDifference >= 1 && DifPlus >= 0 && DifPlus <= 25 || MobDifference > 0 && DifMin >= 0 && DifMin <= 25) {
         chr.dropMessage(-1, "You can feel an evil energy comming, watch yourself!");
         chr.getClient().getSession().write(CField.playSound("ShamanBTTuto/aka"));
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810404), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810405), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810406), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810407), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810408), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        InterServerHandler.now = 0;
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810585), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810586), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810409), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810410), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810411), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810412), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810413), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810416), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810417), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810418), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810419), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810420), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810421), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810422), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810423), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810424), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810425), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810426), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810427), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810428), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810429), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810430), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810431), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810432), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810433), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810434), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810435), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810436), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810414), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810415), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (chr.getLevel() >= monster.getStats().getLevel()) {
         ...
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                   chr.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9810488), Pos);
                    try {
                        Thread.sleep(5000);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
        ...
        if (mobid == 8810018 && mapid == 240060200) { // Horntail
            broadcastMessage(CField.musicChange("Bgm34/InyourPast"));
            ...
            broadcastMessage(CField.musicChange("Bgm35/SurvivetheSwordland"));
...
//            FileoutputUtil.log(FileoutputUtil.Horntail_Log, MapDebug_Log());
         ...
         else if (mobid == 8920100 && mapid == 105200310) {//Queen
            final MapleReactor my = new MapleReactor(MapleReactorFactory.getReactor(1052008), 1052008);
            my.setState((byte) 1);
            spawnReactorOnGroundBelow(my, new java.awt.Point(56, 135));   
        }
        else if (mobid == 8920000 && mapid == 105200710) {
            final MapleReactor my = new MapleReactor(MapleReactorFactory.getReactor(1052006), 1052006);
            my.setState((byte) 1);
            spawnReactorOnGroundBelow(my, new java.awt.Point(56, 135));   
        }
        else if (mobid == 8900100 && mapid == 105200210) {//Pierre
            final MapleReactor my = new MapleReactor(MapleReactorFactory.getReactor(1052008), 1052008);
            my.setState((byte) 1);
            spawnReactorOnGroundBelow(my, new java.awt.Point(495, 551));   
        }
        else if (mobid == 8900000 && mapid == 105200610) {
            final MapleReactor my = new MapleReactor(MapleReactorFactory.getReactor(1052006), 1052006);
            my.setState((byte) 1);
            spawnReactorOnGroundBelow(my, new java.awt.Point(495, 551));   
        }
        else if (mobid == 8910100 && mapid == 105200110) {//VonBon
            final MapleReactor my = new MapleReactor(MapleReactorFactory.getReactor(1052008), 1052008);
            my.setState((byte) 1);
            spawnReactorOnGroundBelow(my, new java.awt.Point(-131, 454));   
        }
        else if (mobid == 8910000 && mapid == 105200510) {
            final MapleReactor my = new MapleReactor(MapleReactorFactory.getReactor(1052006), 1052006);
            my.setState((byte) 1);
            spawnReactorOnGroundBelow(my, new java.awt.Point(-131, 454));   
        }
        else if (mobid == 8930100 && mapid == 105200410) {//Vellum
            final MapleReactor my = new MapleReactor(MapleReactorFactory.getReactor(1052008), 1052008);
            my.setState((byte) 1);
            spawnReactorOnGroundBelow(my, new java.awt.Point(-192, 452));   
        }
        else if (mobid == 8930000 && mapid == 105200810) {
            final MapleReactor my = new MapleReactor(MapleReactorFactory.getReactor(1052006), 1052006);
            my.setState((byte) 1);
            spawnReactorOnGroundBelow(my, new java.awt.Point(-192, 452));   
        }
        else if (mobid == 6500012 && mapid == 231050000) {
            final MapleReactor my = new MapleReactor(MapleReactorFactory.getReactor(2316000), 2316000);
            my.setState((byte) 1);
            spawnReactorOnGroundBelow(my, new java.awt.Point(885, 63));   
            //MapleQuest.getInstance(4325).forceComplete(c.getPlayer(), 0);
            //c.getPlayer().gainExp(150000,true, true, false);
        }
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
    ...
    public final void destroyReactor(final int oid) {
        ...
        if (reactor.getDelay() > 0) {
            ...
            }, reactor.getReactorId() >= 100000 && reactor.getReactorId() <= 200099 ? 300000 /*5 mins for Profession reactors*/: reactor.getDelay());
        }
    }
    ...
    public final void spawnMonster(MapleMonster monster, int spawnType) {
        spawnMonster(monster, spawnType, false, null, this);
    }
   
    public final void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite, final MapleCharacter chr, final MapleMap Map) {      
        if (this.getId() == 109010100 && monster.getId() != 9300166) {
            return;
        } 
        final OverrideMonsterStats overrideStats = new OverrideMonsterStats(monster.getStats().getHp(), monster.getStats().getMp(), monster.getStats().getExp(), false);
        ...
        if (monster.getId() == 8810026) {
         this.killMonster(8810026);
          MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                    monster.spawnRevives(Map);
                    try {
                        Thread.sleep(4800);
                    } catch (InterruptedException ex) {
                        System.out.println("Error spawning monster: " + ex);
         ...
         if (monster.getId() == 8810130) {
         this.killMonster(8810130);
         MapTimer.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                    monster.spawnRevives(Map);
    ...
    public final void spawnMobDrop(final Item idrop, final Point dropPos, final MapleMonster mob, final MapleCharacter chr, final byte droptype, final int questid) {
        ...
        mdrop.setInstanceOwner(idrop.getInstanceOwner());
        if (idrop.getItemId() == 4001473) {
            System.out.print("\r\nidrop: " + idrop + ". dropPos: " + dropPos + ". Mob: " + mob + ". chr: " + chr + ". droptype: " + droptype + ". questid: " + questid);
        }
        spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
            @Override
            public void sendPackets(MapleClient c) {
                if (c != null && c.getPlayer() != null && (questid <= 0 || c.getPlayer().getQuestStatus(questid) == 1) /*&&
                         (idrop.getItemId() / 10000 != 238 || c.getPlayer().getMonsterBook().getLevelByCard(idrop.getItemId()) >= 2)*/ && mob != null && dropPos != null) {
                    if (mdrop.getInstanceOwner() == "" || mdrop.getInstanceOwner() == chr.getName()) {
    ...
    public final void spawnAutoDrop(final int itemid, final Point pos) {
        ...
        if (GameConstants.getInventoryType(itemid) == MapleInventoryType.EQUIP) {
            idrop = ii.randomizeStats((Equip) ii.getEquipById(itemid), null, null);
        } else {
