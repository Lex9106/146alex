...
public class MapleLifeFactory {
    ...
    public static MapleMonsterStats getMonsterStats(int mid) {
        ...
        if (stats == null) {
            ...
            stats.setElite(MapleDataTool.getIntConvert("eliteMob", monsterInfoData, 0) > 0);
            ...
            if (stats.getLevel() < 11 && !stats.isBoss()) {
            stats.setHp(MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0));
            ...
            stats.setExp((int) (MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 6));
            }
            if (stats.getLevel() > 10 && stats.getLevel() < 30 && !stats.isBoss()) {
            stats.setHp(MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 2);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
            stats.setPhysicalAttack((MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0) * 2));
            stats.setMagicAttack((MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0)) * 2);
            stats.setExp(MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 8);
            }
            if (stats.getLevel() > 29 && stats.getLevel() < 70 && !stats.isBoss()) {
            stats.setHp(MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 4);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
            stats.setPhysicalAttack((MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0) * 3));
            stats.setMagicAttack((MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0)) * 3);
            stats.setExp((int) (MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 10));
            }
            if (stats.getLevel() > 69 && stats.getLevel() < 120 && !stats.isBoss() && !stats.isElite()) {
            stats.setHp(MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 8);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
            stats.setPhysicalAttack(MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0) * 4);
            stats.setMagicAttack(MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0) * 4);
            stats.setExp((int) (MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 12));
            }
            if (stats.getLevel() > 119 && stats.getLevel() < 150 && !stats.isBoss() && !stats.isElite()) {
            stats.setHp(MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 12);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
            stats.setPhysicalAttack((MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0) * 3));
            stats.setMagicAttack((MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0)) * 3);
            stats.setExp(MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 14);
            }
            if (stats.getLevel() > 149 && stats.getLevel() < 251 && !stats.isBoss() && !stats.isElite()) {
            stats.setHp(MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 15);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
            stats.setPhysicalAttack(MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0) * 2);
            stats.setMagicAttack(MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0) * 2);
            stats.setExp((int) (MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 20));
            }
            if (stats.getLevel() < 11 && stats.isBoss() && !stats.isElite()) {
            stats.setHp(MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 2);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
            stats.setPhysicalAttack(MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0) * 2);
            stats.setMagicAttack(MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0) * 2);
            stats.setExp((int) (MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 2));
            }
            if (stats.getLevel() > 9 && stats.getLevel() < 30 && stats.isBoss() && !stats.isElite()) {
            stats.setHp(MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 2);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
            stats.setPhysicalAttack(MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0) * 2);
            stats.setMagicAttack(MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0) * 2);
            stats.setExp((int) (MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 3));
            }
            if (stats.getLevel() > 29 && stats.getLevel() < 70 && stats.isBoss() && !stats.isElite()) {
            stats.setHp(MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 2);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
            stats.setPhysicalAttack(MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0) * 2);
            stats.setMagicAttack(MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0) * 2);
            stats.setExp((int) (MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 4));
            }
            if (stats.getLevel() > 69 && stats.getLevel() < 120 && stats.isBoss() && !stats.isElite()) {
            stats.setHp(MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 2);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
            stats.setPhysicalAttack(MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0) * 2);
            stats.setMagicAttack(MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0) * 2);
            stats.setExp((int) (MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 5));
            }
            if (stats.getLevel() > 119 && stats.getLevel() < 150 && stats.isBoss() && !stats.isElite()) {
            stats.setHp(MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 2);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
            ...
            if (stats.getLevel() > 149 && stats.getLevel() < 251 && stats.isBoss() && !stats.isElite()) {
            stats.setHp((long) MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 2);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
            stats.setPhysicalAttack(MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0) * 2);
            stats.setMagicAttack(MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0) * 2);
            stats.setExp((int) (MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 7));
            }
            if (stats.isElite() && !(stats.getId() >= 8220022 && stats.getId() <= 8220026)) {
            int Level = 0;
            if (stats.getLevel() > 69 && stats.getLevel() < 121) {
            Level = 5;   
            }
            if (stats.getLevel() > 119) {
            Level = 3;   
            }
            stats.setHp((long) MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 250);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0) * 100);
            stats.setPhysicalAttack(MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0) * Level);
            stats.setMagicAttack(MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0) * Level);
            stats.setExp((int) (MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 500));
            }
            if (stats.isElite() && (stats.getId() >= 8220022 && stats.getId() <= 8220026)) {
            stats.setHp((long) MapleDataTool.getIntConvert("maxHP", monsterInfoData, 0) * 20);
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0) * 20);
            stats.setPhysicalAttack(MapleDataTool.getIntConvert("PADamage", monsterInfoData, 0));
            stats.setMagicAttack(MapleDataTool.getIntConvert("MADamage", monsterInfoData, 0));
            stats.setExp((int) (MapleDataTool.getIntConvert("exp", monsterInfoData, 0) * 100000));
            }
            ...
            stats.setPushed((int) (stats.getHp() * 5 / 100));
            //if (stats.getLevel() >= 90 && !stats.isBoss()) {
            //FileoutputUtil.log("Mob.txt", "" + mid + ",//" + stats.getName() + "\r\n");
            //}
            ...
            if (stats.isBoss() || isDmgSponge(mid) || stats.isElite()) {
                if (monsterInfoData.getChildByPath("hpTagColor") == null || monsterInfoData.getChildByPath("hpTagBgcolor") == null) {
                    stats.setTagColor(0);
                    stats.setTagBgColor(0);
                    if (stats.isElite() || stats.getId() == 6130101) {
                stats.setTagColor(1);
                stats.setTagBgColor(5);  
                }
                } 
                else {
