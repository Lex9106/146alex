...
public class MapleMonster extends AbstractLoadedMapleLife {
    public static int durationstatus;
    ...
    public final int killBy(final MapleCharacter killer, final int lastSkill) {
        ...
        if (killer != null && stats.isElite()) {
        killer.finishAchievement(49);
        }
        spawnRevives(getMap());
        if (eventInstance != null) {
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if (poison && eff != null) {
            duration = eff.getDOTTime() * 1000;
            duration += from.getStat().dotTime * 1000;
            if (from.isGM()) {
        from.dropMessage(5,"Duration of DoT is: "+ duration / 1000.00 + " secs."); 
        }
        }
        if (!poison) {
        durationstatus = (int) duration + 1000;
        if (from.isGM()) {
        from.dropMessage(5,"Debuff Duration is: "+ duration / 1000.00 + " secs."); 
        }
        }
        if (poison) {   
        ...
        if (durationstatus > 0 && !poison) {//at least it works
            Timer.WorldTimer.getInstance().schedule(new Runnable() {
             @Override
             public void run() {
            cancelSingleStatus(status); //Cancels the poison status on the monster.
            }
         }, durationstatus);
        }
        ...
        if (stat == MonsterStatus.POISON || stat == MonsterStatus.VENOMOUS_WEAPON) {
            ...
            try {
               ...
               if (getController() != null) {
                    map.broadcastMessage(getController(), MobPacket.applyMonsterStatus(this, poisons, (eff.getDOTTime() + from.getStat().dotTime)), getTruePosition());
                    getController().getClient().getSession().write(MobPacket.applyMonsterStatus(this, poisons, (eff.getDOTTime() + from.getStat().dotTime)));
            ...
            TimerTask timeTask = new TimerTask() {
                @Override
                public void run() {
                    if(MapleMonster.this == null || MapleMonster.this.isHidden()){ //If the monster only has one HP, or is null or hidden, cancel the timer to preserve resources.
                        DoTTimer.cancel();
                    }
                    if (isAlive())  { 
                        doPoison(status, new WeakReference<>(from)); //Damages the monster.
                        DoTTimer.purge();
                    }  
                    if((runCount.incrementAndGet() >= (durationFinal/1000)) || !isAlive()){ //When duration is met, or monster is dead, remove the timer.
                         //status.cancelPoisonSchedule(MapleMonster.this);  //Cancels the poison status on the monster
                         cancelSingleStatus(status); //Cancels the poison status on the monster.
            ...
            DoTTimer.schedule(timeTask, 0, 1015);
                    ...
                    map.broadcastMessage(MobPacket.applyMonsterStatus(this, poisons, (eff.getDOTTime() + from.getStat().dotTime)), getTruePosition());
                }
            } finally {
    ...
    public final void applyMonsterBuff(final Map<MonsterStatus, Integer> effect, final int skillId, final long duration, final MobSkill skill, final List<Integer> reflection) {
        ...
        if (reflection.size() > 0) {
            if (con.isSuperGM()) {
            con.dropMessage(5,"Reflect has been casted.");
            }
    ...
    public final void doPoison(final MonsterStatusEffect status, final WeakReference<MapleCharacter> weakChr) {
        ...
        if (!cancel) {
            ...
            if (chr.isSuperGM()) {
            chr.dropMessage(5,"DoT Damage : "+ damage);
            }
            if (shadowWeb) {
            ...
            for (MonsterDropEntry d : dropEntry) { //set to 4x rate atm, 40% chance + 10x
                if (d.itemId > 0 && d.questid == 0 && d.itemId / 10000 != 238 && Randomizer.nextInt(999999) < (int) (10 * d.chance * chServerrate * chr.getDropMod() * chr.getStat().dropBuff / 100.0 * (showdown / 100.0))) { //kinda op
                    if (GameConstants.getInventoryType(d.itemId) == MapleInventoryType.EQUIP) {
                        ...
                        idrop = MapleItemInformationProvider.getInstance().randomizeStats(eq, chr, chr.getClient());
                    } else {
