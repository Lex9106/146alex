    ...
    private void loadDrop(final int monsterId) {
        ...
        final MapleMonsterStats mons = MapleLifeFactory.getMonsterStats(monsterId);
            ...
            while (rs.next()) {
                ...
                chance = rs.getInt("chance");
                if (GameConstants.getInventoryType(itemid) == MapleInventoryType.EQUIP && !mons.isBoss()) {
                    chance *= 15;
                }
                if (GameConstants.getInventoryType(itemid) == MapleInventoryType.EQUIP && mons.isBoss() && !GameConstants.isBossExclusiveItem(itemid) && !(mons.getId() >= 8610005 && mons.getId() <= 8610027)) {
                    chance = 50000;
                }
                if (GameConstants.getInventoryType(itemid) == MapleInventoryType.USE && !GameConstants.isRecipe(itemid) && !GameConstants.isUpgradeScroll(itemid) && !GameConstants.isChaosScroll(itemid)) {
                    chance *= 3;
                }
                if (GameConstants.getInventoryType(itemid) == MapleInventoryType.USE && GameConstants.isRecipe(itemid)) {
                    chance = 1000;
                }
                if (GameConstants.isUpgradeScroll(itemid)) {
                    chance *= 5;
                }
                if (GameConstants.isChaosScroll(itemid)) {
                    chance *= 2;
                }
    ...
    public void addExtra() {
        ...
        for (Entry<Integer, ArrayList<MonsterDropEntry>> e : drops.entrySet()) {
            ...
            if (item != null && item.intValue() > 0) {
                e.getValue().add(new MonsterDropEntry(item.intValue(), mons.isBoss() ? 1000000 : 100000, 1, 1, 0));
            }
            StructFamiliar f = ii.getFamiliarByMob(e.getKey().intValue());
            if (f != null) {
                e.getValue().add(new MonsterDropEntry(f.itemid, 1000, 1, 1, 0));
            }
        }
        for (Entry<Integer, Integer> i : ii.getMonsterBook().entrySet()) {
            if (!drops.containsKey(i.getKey())) {
                ...
                if (f != null) {
                    e.add(new MonsterDropEntry(f.itemid, mons.isBoss() ? 3000 : 1000, 1, 1, 0));
                }
                addMeso(mons, e);
        ...
        for (StructFamiliar f : ii.getFamiliars().values()) {
            if (!drops.containsKey(f.mob)) {
                MapleMonsterStats mons = MapleLifeFactory.getMonsterStats(f.mob);
                ArrayList<MonsterDropEntry> e = new ArrayList<>();
                e.add(new MonsterDropEntry(f.itemid, 1000, 1, 1, 0));
                addMeso(mons, e);
                drops.put(f.mob, e);
            }
        }
