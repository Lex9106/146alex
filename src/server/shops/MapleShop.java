...
public class MapleShop {
    ...
    public void buy(MapleClient c, int itemId, short quantity) {
        ...
        if (index >= 0) {
            final Item i = c.getPlayer().getRebuy().get(index);//rebuy
            ...
            if (price >= 0 && c.getPlayer().getMeso() >= price) {
                if (MapleInventoryManipulator.checkSpace(c, itemId, i.getQuantity(), i.getOwner())) {
                    ...
                    MapleInventoryManipulator.addbyItemRebuy(c, i);
        ...
        if (item != null && item.getPrice() > 0 && item.getReqItem() == 0) {
            ...
            if (price >= 0 && c.getPlayer().getMeso() >= price) {
                ...
                if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
                 ...
                 //FileoutputUtil.log("BuyItems.txt", "" + item.getItemId() + ",//" + msg + "\r\n");
                 MapleInventoryManipulator.addbyId_Gachapon(c, itemId, (short)(quantity * stockQuantity));
        ...
        if ((quantity <= iQuant) && (iQuant > 0)) {
            if (GameConstants.GMS) {
                if (item.getQuantity() == quantity) {
                    if (c.getPlayer().getRebuy().size() < 150) {
                        c.getPlayer().getRebuy().add(item.copy());
                    } else if (c.getPlayer().getRebuy().size() == 150) {
                        for (int i = 1; i < 10; i++) {
