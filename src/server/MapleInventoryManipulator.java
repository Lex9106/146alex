...
public class MapleInventoryManipulator {
    ...
    public static short addbyItem(final MapleClient c, final Item item, final boolean fromcs) {
        ...
        if (type.equals(MapleInventoryType.EQUIP) && !ii.isCash(item.getItemId())) {
        final String msg = ii.getName(item.getItemId());
        c.getPlayer().dropMessage(-5,"Add by Item Id : "+ item.getItemId()+ " Name: "+msg);
        }
        }
        final MapleInventoryType type = GameConstants.getInventoryType(item.getItemId());
        ...
        if (GameConstants.getInventoryType(item.getItemId()) == MapleInventoryType.EQUIP && item.getUniqueId() >= 0) {
        ...
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        ii.randomizeStats((Equip) ii.getEquipById(item.getItemId()), c.getPlayer(), c.getPlayer().getClient());
        final Equip eq = (Equip) item;
        final int reqLevel = ii.getReqLevel(eq.getItemId());
        if (eq.getState() == 0 && !ii.isCash(eq.getItemId())) {
        if (reqLevel >= 10 && !GameConstants.isTotem(eq.getItemId())) {
        eq.resetPotential();
        }
        if (reqLevel <= 59 && eq.getUpgradeSlots() > 0 && !GameConstants.isMechanic(eq.getItemId()) && !GameConstants.isDragon(eq.getItemId()) && !GameConstants.isMedal(eq.getItemId())) {
        eq.setSocket1(0);
        }
    ...
    public static boolean addbyItem_Craft(final MapleClient c, final Item item) {
        return addbyItem_Craft(c, item, false) >= 0;
    }

    public static short addbyItem_Craft(final MapleClient c, final Item item, final boolean fromcs) {
        if (c.getPlayer().isGM()) {
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        final MapleInventoryType type = GameConstants.getInventoryType(item.getItemId());
        if (type.equals(MapleInventoryType.EQUIP) && !ii.isCash(item.getItemId())) {
        final String msg = ii.getName(item.getItemId());
        c.getPlayer().dropMessage(-5,"Add by Item Id _ Craft : "+ item.getItemId()+ " Name: "+msg);
        }
        }
        final MapleInventoryType type = GameConstants.getInventoryType(item.getItemId());
        final short newSlot = c.getPlayer().getInventory(type).addItem(item);
    ...
    public static boolean addbyItemRebuy(final MapleClient c, final Item item) {
        return addbyItemRebuy(c, item, false) >= 0;
    }

    public static short addbyItemRebuy(final MapleClient c, final Item item, final boolean fromcs) {
        if (c.getPlayer().isGM()) {
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        final MapleInventoryType type = GameConstants.getInventoryType(item.getItemId());
        if (type.equals(MapleInventoryType.EQUIP) && !ii.isCash(item.getItemId())) {
        ...
        if (GameConstants.getInventoryType(item.getItemId()) == MapleInventoryType.EQUIP && item.getUniqueId() >= 0) {
         if (c.getPlayer().getInventory(MapleInventoryType.EQUIP).getNextFreeSlot() == 2) {
                c.getSession().write(InventoryPacket.getInventoryFull());
                c.getSession().write(InventoryPacket.getShowInventoryFull());
                    }
        final Equip eq = (Equip) item;
    ...
    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, boolean hours, String gmLog) {
        if (c.getPlayer().isGM()) {
        c.getPlayer().dropMessage(5,"Add id");  
        }
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (quantity == 1) {
                ...
                if (GameConstants.isMedal(itemId)) {
                final Equip eq = (Equip) nEquip;
                eq.resetPotential();
                }
                InventoryHandler.Reveal(nEquip , c);
        final Equip eq = (Equip) nEquip;
        ii.randomizeStats(eq, c.getPlayer(), c);
        if (eq.getState() == 0) {
        final int reqLevel = ii.getReqLevel(eq.getItemId());
        if (reqLevel >= 10 && !GameConstants.isTotem(eq.getItemId())) {
        eq.resetPotential();
    ...
    public static Item addbyId_Gachapon(final MapleClient c, final int itemId, short quantity) {
        ...
        if (c.getPlayer().isGM()) {
        c.getPlayer().dropMessage(5,"Add by id gachapon"); 
        }
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        if ((ii.isPickupRestricted(itemId) && c.getPlayer().haveItem(itemId, 1, true, false)) || (!ii.itemExists(itemId))) {
            c.getSession().write(InventoryPacket.getInventoryFull());
        ...
        final Item item = ii.randomizeStats((Equip) ii.getEquipById(itemId), c.getPlayer(), c.getPlayer().getClient());
        if (quantity == 1) {
        final Equip eq = (Equip) item;
                final short newSlot = c.getPlayer().getInventory(type).addItem(item);
                if (newSlot == -1) {
    ...
    public static Item addbyId_MagicWheel(final MapleClient c, final int itemId, short quantity) {
        if (c.getPlayer().getInventory(MapleInventoryType.EQUIP).getNextFreeSlot() == -1 || c.getPlayer().getInventory(MapleInventoryType.USE).getNextFreeSlot() == -1 || c.getPlayer().getInventory(MapleInventoryType.ETC).getNextFreeSlot() == -1 || c.getPlayer().getInventory(MapleInventoryType.SETUP).getNextFreeSlot() == -1) {
            return null;
        }
        if (c.getPlayer().isGM()) {
        c.getPlayer().dropMessage(5,"Add by id gachapon wheel"); 
        }
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        if ((ii.isPickupRestricted(itemId) && c.getPlayer().haveItem(itemId, 1, true, false)) || (!ii.itemExists(itemId))) {
            c.getSession().write(InventoryPacket.getInventoryFull());
        ...
        final Item item = ii.randomizeStatsWheel((Equip) ii.getEquipById(itemId), c.getPlayer(), c.getPlayer().getClient());
        if (quantity == 1) {
        final Equip eq = (Equip) item;
                final short newSlot = c.getPlayer().getInventory(type).addItem(item);
                if (newSlot == -1) {
    ...
    public static boolean addFromDrop(final MapleClient c, Item item, final boolean show, final boolean enhance) {
        if (c.getPlayer().isGM()) {
        c.getPlayer().dropMessage(5,"Add from drop");  
        }
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
    ...
    public static boolean addFromDrop_Craft(final MapleClient c, Item item, final boolean show, final boolean enhance) {
        if (c.getPlayer().isGM()) {
        c.getPlayer().dropMessage(5,"Add from drop _ Craft");  
        }
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
    ...
    public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP) && initialTarget != null
            ...
            if ((olddstQ + oldsrcQ) > slotMax) {
                System.out.println("idk " + dst + " " + source.getItemId());
        ...
        System.out.println("Merged from Equip to Inventoty " + dst + " " + source.getItemId());
            ...
            System.out.println("Simple move " + dst + " " + source.getItemId());
    ...
    // }
        ...
        if (!GameConstants.isDemonAvenger(chr.getJob()) && !ii.canEquip(stats, source.getItemId(), chr.getLevel(), chr.getJob(), chr.getFame(), statst.getTotalStr(), statst.getTotalDex(), statst.getTotalLuk(), statst.getTotalInt(), c.getPlayer().getStat().levelBonus, source.getReqLevel())) {
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        if (GameConstants.isDemonAvenger(chr.getJob()) && !ii.canEquipDemon(stats, source.getItemId(), chr.getLevel(), chr.getJob(), chr.getFame(), statst.getTotalLuk(), statst.getTotalInt(), c.getPlayer().getStat().levelBonus, source.getReqLevel())) {
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        ...
        if (source.getItemId() / 1000 == 1112 || source.getItemId() / 1000 == 1113) { //ring
    ...
    if (chr.hasEquipped(1122380) || chr.hasEquipped(1122385) || chr.hasEquipped(1122390) || chr.hasEquipped(1122395) || chr.hasEquipped(1122400) || chr.hasEquipped(1122405) || chr.hasEquipped(1122410) || chr.hasEquipped(1122415)) {
    if (source.getItemId() == 1122385 && chr.getBuffedValue(MapleBuffStat.PVP_DAMAGE) != null) {
        chr.dispelBuff(2023251);
        }
        if (source.getItemId() == 1122390 && chr.getBuffedValue(MapleBuffStat.ACC) != null) {
        chr.dispelBuff(2023252);
        }
        if (source.getItemId() == 1122395 && chr.getBuffedValue(MapleBuffStat.PVP_FLAG) != null) {
        chr.dispelBuff(2023253);
        }
        if (source.getItemId() == 1122400 && chr.getBuffedValue(MapleBuffStat.INVINCIBLE) != null) {
        chr.dispelBuff(2023254);
        }
        if (source.getItemId() == 1122405 && chr.getBuffedValue(MapleBuffStat.CRIT_DAMAGE) != null) {
        chr.dispelBuff(2023255);
        }
        if (source.getItemId() == 1122410 && chr.getBuffedValue(MapleBuffStat.INVINCIBILITY) != null) {
        chr.dispelBuff(2023256);
        }
        if (source.getItemId() == 1122415 && chr.getBuffedValue(MapleBuffStat.VIRTUE_EFFECT) != null) {
        chr.dispelBuff(2023257);
        }
        if (source.getItemId() == 1122380 && chr.getBuffedValue(MapleBuffStat.NO_SLIP) != null) {
        chr.dispelBuff(2023258);
        }
    }
    ...
    chr.equipChanged();
     if (!c.getPlayer().hasEquipped(1122385) && c.getPlayer().getBuffedValue(MapleBuffStat.PVP_DAMAGE) != null) {
        c.getPlayer().dispelBuff(2023251);
        }
        if (!c.getPlayer().hasEquipped(1122390) && c.getPlayer().getBuffedValue(MapleBuffStat.ACC) != null) {
        c.getPlayer().dispelBuff(2023252);
        }
        if (!c.getPlayer().hasEquipped(1122395) && c.getPlayer().getBuffedValue(MapleBuffStat.PVP_FLAG) != null) {
        c.getPlayer().dispelBuff(2023253);
        }
        if (!c.getPlayer().hasEquipped(1122400) && c.getPlayer().getBuffedValue(MapleBuffStat.INVINCIBLE) != null) {
        c.getPlayer().dispelBuff(2023254);
        }
        if (!c.getPlayer().hasEquipped(1122405) && c.getPlayer().getBuffedValue(MapleBuffStat.CRIT_DAMAGE) != null) {
        c.getPlayer().dispelBuff(2023255);
        }
        if (!c.getPlayer().hasEquipped(1122410) && c.getPlayer().getBuffedValue(MapleBuffStat.INVINCIBILITY) != null) {
        c.getPlayer().dispelBuff(2023256);
        }
        if (!c.getPlayer().hasEquipped(1122415) && c.getPlayer().getBuffedValue(MapleBuffStat.VIRTUE_EFFECT) != null) {
        c.getPlayer().dispelBuff(2023257);
        }
        if (!c.getPlayer().hasEquipped(1122380) && c.getPlayer().getBuffedValue(MapleBuffStat.NO_SLIP) != null) {
        c.getPlayer().dispelBuff(2023258);
        }
}
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        if (source.getItemId() == 1122385 && c.getPlayer().getBuffedValue(MapleBuffStat.PVP_DAMAGE) != null) {
        c.getPlayer().dispelBuff(2023251);
        }
        if (source.getItemId() == 1122390 && c.getPlayer().getBuffedValue(MapleBuffStat.ACC) != null) {
        c.getPlayer().dispelBuff(2023252);
        }
        if (source.getItemId() == 1122395 && c.getPlayer().getBuffedValue(MapleBuffStat.PVP_FLAG) != null) {
        c.getPlayer().dispelBuff(2023253);
        }
        if (source.getItemId() == 1122400 && c.getPlayer().getBuffedValue(MapleBuffStat.INVINCIBLE) != null) {
        c.getPlayer().dispelBuff(2023254);
        }
        if (source.getItemId() == 1122405 && c.getPlayer().getBuffedValue(MapleBuffStat.CRIT_DAMAGE) != null) {
        c.getPlayer().dispelBuff(2023255);
        }
        if (source.getItemId() == 1122410 && c.getPlayer().getBuffedValue(MapleBuffStat.INVINCIBILITY) != null) {
        c.getPlayer().dispelBuff(2023256);
        }
        if (source.getItemId() == 1122415 && c.getPlayer().getBuffedValue(MapleBuffStat.VIRTUE_EFFECT) != null) {
        c.getPlayer().dispelBuff(2023257);
        }
        if (source.getItemId() == 1122380 && c.getPlayer().getBuffedValue(MapleBuffStat.NO_SLIP) != null) {
        c.getPlayer().dispelBuff(2023258);
        }
