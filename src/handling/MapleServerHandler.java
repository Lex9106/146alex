...
import static handling.world.World.handleMap;
import java.io.File;
...
public class MapleServerHandler extends IoHandlerAdapter implements MapleServerHandlerMBean {
    ...
    private final static List<ChannelServer> cservs = new ArrayList<>(CHANNELS_PER_THREAD);
    ...
    public static void addIP(String theIP) {
        //try {
            //FileWriter fw = new FileWriter(new File("PacketLog_" + theIP + ".txt"), false);
...
// <editor-fold defaultstate="collapsed" desc="Packet Log Implementation">
    ...
    public static void initiate() {
        reloadLoggedIPs();
        RecvPacketOpcode[] block = new RecvPacketOpcode[]{RecvPacketOpcode.NPC_ACTION, RecvPacketOpcode.MOVE_PLAYER, RecvPacketOpcode.PONG, RecvPacketOpcode.MOVE_PET, RecvPacketOpcode.MOVE_SUMMON, RecvPacketOpcode.MOVE_DRAGON, RecvPacketOpcode.MOVE_ANDROID, RecvPacketOpcode.HEAL_OVER_TIME, RecvPacketOpcode.STRANGE_DATA, RecvPacketOpcode.AUTO_AGGRO, RecvPacketOpcode.CANCEL_DEBUFF, RecvPacketOpcode.MOVE_FAMILIAR};
    ...
    public static void handlePacket(final RecvPacketOpcode header, final LittleEndianAccessor slea, final MapleClient c) throws Exception {
        if (ServerConfig.logPackets && !isSpamHeader(header)) {
            ...
            System.out.println("[R]\t" + header.name() + tab + "|\t" + header.getValue() + "\t|\t" + HexTool.getOpcodeToString(header.getValue()));
        ...
        switch (header) {
            case LOGIN_REDIRECTOR:
                ...
                break;
            case CLIENT_HELLO:
                System.out.println(c.getSessionIPAddress() + " IP has been connected!");
                break;
            case PONG:
                c.pongReceived();
                break;
...
//                System.out.println("[CLIENT ERROR] Error Code " + errortype + " Type " + type_str + "\r\n\t[Data] Length " + data_length + " [" + SendPacketOpcode.getOpcodeName(opcodeheader) + " | " + opcodeheader + "]\r\n" + HexTool.toString(slea.read((int) slea.available())));
            ...
            case PLAYER_LOGGEDIN:
                ...
                if (c.getPlayer().hasEquipped(1113020)) {
                SkillFactory.getSkill(80001262).getEffect(1).applyTo(c.getPlayer());
                }
                break;
            case ENTER_PVP:
            ...
            case HEAL_OVER_TIME:
                ...
                break;
            case LINK_SKILL:
                c.getPlayer().GiveLinkedSkill(slea, c);
