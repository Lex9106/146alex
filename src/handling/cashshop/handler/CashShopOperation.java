...
import static handling.channel.handler.InventoryHandler.RevealCS;
...
//            chr.modifyCSPoints(type, -toCharge, true);
        ...
        } else if (action == 14) { //move item to csinventory in shop
...
//            Item item = MapleItemInformationProvider.getInstance().getEquipById(item);
            if (/*item != null && */item.getQuantity() > 0 && MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), item.getOwner())) {
                ...
                final MapleInventoryType type = GameConstants.getInventoryType(item_.getItemId());
                if (type == MapleInventoryType.EQUIP) {
                final Equip eq = (Equip) item_;
                eq.resetPotential();
                RevealCS(eq,c,true);
                }
                if (pos >= 0) {
            ...
            if (item.getQuantity() > 0 && item.getUniqueId() > 0 && c.getPlayer().getCashInventory().getItemsSize() < 100) {
                Item item_ = item.copy();
