...
public class InterServerHandler {
    public static transient int tittleHPMP, EliteBossTime, ElunaBuffTime, ElunaBuffTime2, ElunaBuffTime3, ElunaBuffTime4, ElunaBuffTime5, ElunaBuffTime6, ElunaBuffTime7, ElunaBuffTime8,ShadowShellCount,FamiliarIdLogin, RemoveFamiliarByClick, AutoItem, AutoMeso, PickAll, runningLight, runningLightSlot, runningDark, runningDarkSlot;
    public static transient long now, LastEliteBoss, LastElunaBuff, LastElunaBuff2, LastElunaBuff3, LastElunaBuff4, LastElunaBuff5, LastElunaBuff6, LastElunaBuff7, LastElunaBuff8;
    public static void EnterCS(final MapleClient c, final MapleCharacter chr) {
        ...
        System.out.println("STORED ALL BUFFS/COOLDOWNS IN ENTER_CS");
...
//            c.getSession().write(CWvsContext.BuffPacket.cancelBuff());
  ...
  //      c.getSession().write(JobPacket.addStolenSkill());
        ...
        //PlayerBuffStorage.addDiseaseToStorage(player.getId(), player.getAllDiseases());
        player.getMap().addPlayer(player);
        player.giveCoolDowns(PlayerBuffStorage.getCooldownsFromStorage(player.getId()));
        player.silentGiveBuffs(PlayerBuffStorage.getBuffsFromStorage(player.getId()));
        player.giveSilentDebuff(PlayerBuffStorage.getDiseaseFromStorage(player.getId()));
        System.out.println("APPLIED ALL STORED BUFFS/COOLDOWNS");
        ...
        if (GameConstants.isDemonAvenger(player.getJob())) {
            c.getSession().write(AvengerPacket.giveAvengerHpBuff(player.getStat().getHp() / 2));
        }
        player.spawnClones();
        player.spawnSavedPets();
        if (player.getStat().equippedSummon > 0 && player.getStat().equippedSummon != 80001262) {
