...
public static int TokenId;
   ...
   /*     final List<Integer> maps = new ArrayList<>();
      ...
      //  player.getClient().getChannelServer().getEventSM().getEventManager("EvolutionLab"); //Coming Soon
       ...
       /* int skillid = slea.readInt();
        ...
        int skillid3 = slea.readInt();
        int skillid4 = slea.readInt();
        int skillid5 = (int) slea.readLong();
        System.out.println("SkillId is " + skillid3 + " " + skillid4 + " " + skillid5);
        if (skillid == 3121013) {*/
   ...
   //     }
    ...
    public static void GachExp(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.getPlayer().gainExp(c.getPlayer().getGachExp() * GameConstants.getExpRate_Quest(c.getPlayer().getStat()), true, true, false);
    ...
    public static void magicWheel(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == 2) {
            ...
            c.getPlayer().dropMessage(5,"Token Id is " + tokenId);
            }
            GameConstants.loadWheelRewards(items, tokenId, c.getPlayer());
            int end = Randomizer.nextInt(9);
            ...
            int Quantity = 0;
            if (item == 2028066 || item == 2028067) { 
            Quantity = Randomizer.nextInt(20);
            }
            if (item / 1000 == 4023) {
            ...
            if (item / 1000 == 4021 && item != 4021010) {
            Quantity = Randomizer.nextInt(10);
            }
            Item item2 = MapleInventoryManipulator.addbyId_MagicWheel(c, item, (short) (Quantity > 0 ? Quantity : 1));
            byte rareness = 0;
            if (rareness == 0) {
            rareness = GameConstants.gachaponRareItem(item);
      }
      if (rareness > 0) {
        World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(c.getPlayer().getName(), " : got a(n)", item2, rareness, "Magic Wheel"));
      }
      final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
      final String msg = ii.getName(item);
            c.getPlayer().setWheelItem(0);
            if (Quantity == 0) {
            c.getPlayer().dropMessage(-5, "You have obtained " + msg);
            }
            else {
            c.getPlayer().dropMessage(-5, "You have obtained " + msg + " x" + Quantity);    
            }
            ...
            switch (type) {
                case 1:
                    if (MapleInventoryManipulator.checkSpace(c, itemId, 1, "")) {
                        c.getPlayer().gainItem(itemId, c.getPlayer().getQuestStatus(1628) == 2 && c.getPlayer().getQuestStatus(1629)!=2 && itemId == 4310029 ? 10 : 1);
                        c.getPlayer().deleteReward(id);
                        quantity = 1;
                        msg = 0x0C;
                    } else {
                    ...
                    break;
                case 3:
                    if (c.getPlayer().getCSPoints(1) + mp >= 0) {
                        c.getPlayer().modifyCSPoints(1, mp, false);
                        c.getPlayer().deleteReward(id);
                        quantity = mp;
                        msg = 0x0B;
                    } else {
