...
public class ItemMakerHandler {
    ...
    static {
        ...
        craftingEffects.put("Effect/BasicEff.img/professions/equip_product", 92010000);
   ...
   public static final void StartHarvest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
    ...
    public static final void CraftComplete(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        int fail = 1, toGet = 0, toGetSpecial = 0, expGain = 0, fatigue = 0;
        short quantity = 1;
        ...
        if (craftID == 92049000) { //disassembling
            ...
            if (reqLevel >= 105) {
            toGetSpecial = 4021021;
            }
            if (quantity <= 5) {
               cr = CraftRanking.GOOD;
            }
            ...
            fatigue = 3;
            if (toGetSpecial > 0 && Randomizer.nextInt(100) <= 15) {
            MapleInventoryManipulator.addById(c, toGetSpecial, quantity, "Made by disassemble " + itemId + " on " + FileoutputUtil.CurrentReadable_Date());
            }
            ...
            MapleInventoryManipulator.addbyItem_Craft(c, newEquip);
        } else {
                        ...
                        if (Randomizer.nextInt(100) < i.right) {
                            ...
                            if (GameConstants.getInventoryType(toGet) == MapleInventoryType.EQUIP) {
                                ...
                                /*if (Randomizer.nextInt(100) < (theLevl * 2)) {
                                    first = (Equip) ii.randomizeStats(first, c.getPlayer(), c.getPlayer().getClient());
                                    cr = CraftRanking.COOL;
                                }
                                if (Randomizer.nextInt(100) < (theLevl * (first.getUpgradeSlots() > 0 ? 2 : 1))) {
                                    first = (Equip) ii.randomizeStats(first, c.getPlayer(), c.getPlayer().getClient());
                                    first.resetPotential();
                                    InventoryHandler.Reveal(first,c.getPlayer().getClient());
                                    cr = CraftRanking.COOL;
                                }*/
                                cr = CraftRanking.COOL;
                                first = (Equip) ii.randomizeStats(first, c.getPlayer(), c.getPlayer().getClient());
                            ...
                            MapleInventoryManipulator.addFromDrop_Craft(c, receive, true, false);
                            fail = 0;
                            if (ce.needOpenItem) {
        ...
        if (expGain > 0 && theLevl < 10) {
            ...
            chr.dropMessage(-5, s + "'s mastery increased. (+" + expGain / 10 + ")");
            if (chr.addProfessionExp((craftID / 10000) * 10000, expGain / 10)) {
        ...
        chr.getMap().broadcastMessage(CField.craftFinished(chr.getId(), craftID, cr.i, fail > 0 ? 1 : 0,toGet, quantity, expGain / 10));
    }
    ...
    public static final void RewardPot(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.getPlayer().gainItem(5062000, 3);
        }
        else if (Randomizer.nextInt(100) < 50) {
        c.getPlayer().gainItem(5062001, 3);
        }
        else if (Randomizer.nextInt(100) < 30) {
        c.getPlayer().gainItem(5062002, 3);
        }
        else if (Randomizer.nextInt(100) < 20) {
        c.getPlayer().gainItem(5062005, 3);
        }
        else if (Randomizer.nextInt(100) < 10) {
        c.getPlayer().gainItem(5062006, 3);
        }
