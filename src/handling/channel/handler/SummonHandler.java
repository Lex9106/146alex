...
public static final void MoveDragon(final LittleEndianAccessor slea, final MapleCharacter chr) {//MIXTAMAL6 +Mally
    ...
    public static final void SubSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        if (GameConstants.isAngel(sum.getSkill()) || GameConstants.isLightningGod(sum.getSkill())) {
            if (sum.getSkill() == 80001262) {
                MapleItemInformationProvider.getInstance().getItemEffect(2023189).applyTo(chr);
            }
            else if (sum.getSkill() % 10000 == 1087) {
            ...
            int value = chr.getBuffedValue(MapleBuffStat.ANGELICBLESSING);
            chr.changeSkillLevel(SkillFactory.getSkill(70000078), (byte) value,(byte) 0); 
        }
    }
