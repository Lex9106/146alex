...
public class MobHandler {
    public static final void MoveMonster(final LittleEndianAccessor rh, final MapleClient c, final MapleCharacter chr) {
        ...
        int realskill = 0;
        int level = 0;
        //int realskill = rh.readByte() & 0xFF;//skill_1
        //int level = rh.readByte() & 0xFF;//skill_2
        //short option = rh.readShort();//skill_3,skill_4  
       if (useSkill) {
                        ...
                        if (reqHp <= mobSkill.getHP()) {
                            ...
                            if (mobSkill.getCoolTime() == 0) {
                                mobSkill.applyEffect(chr, monster, true);
                                 if (chr.isSuperGM()) {
                                            chr.dropMessage(-6,"Skill id casted is " + mobSkill.getSkillId());    
                                            }
                            } else {
    ...
    public static final void SpawnFamiliar(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (c.getPlayer().summonedFamiliar != null) {
        c.getPlayer().dispelBuff(ii.getFamiliar(c.getPlayer().summonedFamiliar.getFamiliar()).passive); 
        FamiliarIdLogin = 0;
        }
        ...
        c.getPlayer().dropMessage(5,"Familiar Id " + /*chr.getSummonedFamiliar().getFamiliar()*/ mId + "/ Passive ID is " + ii.getFamiliar(c.getPlayer().summonedFamiliar.getFamiliar()).passive);
        }
            }
        }
    }
    
    public static final void SpawnFamiliarChannel(int FamiliarID, MapleClient c, MapleCharacter chr) {
        int mId = FamiliarID;
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        MonsterFamiliar mf = (MonsterFamiliar) c.getPlayer().getFamiliars().get(mId);
        c.getPlayer().spawnFamiliar(mf, false);
         if (c.getPlayer().isSuperGM()) {
        c.getPlayer().dropMessage(5,"Spawned Familiar Id " + mf + "/ Passive ID is " + ii.getFamiliar(c.getPlayer().summonedFamiliar.getFamiliar()).passive);
        }
    }
        ...
        for (Triple attack : attackPair) {
            ...
            if ((mons != null) && (mons.isAlive()) && (!mons.getStats().isFriendly()) && (mons.getLinkCID() <= 0) && (((List) attack.right).size() <= f.attackCount)) {
                ...
                for (Iterator i$ = ((List) attack.right).iterator(); i$.hasNext();) {
                    int damageSlea = (((Integer) i$.next()).intValue());
                    int damage = (int) (damageSlea * (chr.getStat().getCurrentMaxBaseDamage() / 4000));
                    int totaldamage = (damageSlea + damage);
                    chr.getMap().broadcastMessage(chr, MobPacket.damageMonster(mons.getObjectId(), damage), true);
                    mons.damage(chr, totaldamage, true);
                    chr.checkMonsterAggro(mons);
                    chr.getClient().getSession().write(CField.playSound("Cokeplay/Hit"));
                    if (chr.isSuperGM()) {
                    chr.dropMessage(5,"Skill id is " + skillid); 
                    chr.dropMessage(5,"Attack Familiar " + totaldamage);
                    }
                }
                if ((f.makeChanceResult()) && (mons.isAlive())) {
                    ...
                    final MapleStatEffect venomEffect = skill.getEffect(chr.getSkillLevel(skill) + 5);
                    mons.applyStatus(chr, new MonsterStatusEffect(s, Integer.valueOf(f.speed), MonsterStatusEffect.genericSkill(s), null, false), true, venomEffect.getDOTTime(), true, venomEffect);
                    }
                    if (skillid == 810 || skillid == 811 || skillid == 868 || skillid == 873) {
                    final Skill skill = SkillFactory.getSkill(2111004);
                    final MapleStatEffect venomEffect = skill.getEffect(chr.getSkillLevel(skill) + 10);
                    mons.applyStatus(chr, new MonsterStatusEffect(s, Integer.valueOf(f.speed), MonsterStatusEffect.genericSkill(s), null, false), true, venomEffect.getDOTTime(), true, venomEffect);
                    }
                    if (skillid == 812 || skillid == 821 || skillid == 825 || skillid == 842 || skillid == 855 || skillid == 877 || skillid == 882 || skillid == 887 || skillid == 900) {
                    final Skill skill = SkillFactory.getSkill(2111004);
                    final MapleStatEffect venomEffect = skill.getEffect(chr.getSkillLevel(skill) + 15);
                    mons.applyStatus(chr, new MonsterStatusEffect(s, Integer.valueOf(f.speed), MonsterStatusEffect.genericSkill(s), null, false), true, venomEffect.getDOTTime(), true, venomEffect);
                    }
                    if (skillid == 847 || skillid == 857 || skillid == 903) {
                    final Skill skill = SkillFactory.getSkill(2111004);
                    final MapleStatEffect venomEffect = skill.getEffect(chr.getSkillLevel(skill) + 20);
                    mons.applyStatus(chr, new MonsterStatusEffect(s, Integer.valueOf(f.speed), MonsterStatusEffect.genericSkill(s), null, false), true, venomEffect.getDOTTime(), true, venomEffect);
                    }
                    }
                    //
    ...
    public static final void TouchFamiliar(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if ((!target.getStats().isFriendly()) && (chr.getCheatTracker().checkFamiliarAttack(chr))) {
            ...
            chr.getMap().broadcastMessage(chr, MobPacket.damageMonster(target.getObjectId(), damage), true);
            target.damage(chr, totaldamage, true);
            chr.checkMonsterAggro(target);
            chr.getClient().getSession().write(CField.playSound("Cokeplay/Hit"));
             if (chr.isSuperGM()) {
                    chr.dropMessage(5,"Touch Familiar " + totaldamage);
                    }
    ...
    public static final void UseFamiliar(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        c.getSession().write(CWvsContext.getFamiliarInfo(c.getPlayer()));
        return;
    }
}
