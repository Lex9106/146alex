...
public class MapleQuestStatus implements Serializable {
    ...
    public final boolean mobKilled(final int id, final int skillID, MapleCharacter chr) {
        ...
        if (mob != null) {
            ...
            if (chr.hasEquipped(1142323)) {
            killedMobs.put(id, Math.min(mob + 2, mo));
            }
            else if (chr.hasEquipped(1142324)) {
            killedMobs.put(id, Math.min(mob + 3, mo));
            }
            else if (chr.hasEquipped(1142325)) {
            killedMobs.put(id, Math.min(mob + 4, mo));
            }
            else if (chr.hasEquipped(1142326)) {
        ...
        for (Entry<Integer, Integer> mo : killedMobs.entrySet()) {
            if (questCount(mo.getKey(), id)) {
                ...
                if (chr.hasEquipped(1142323)) {
            killedMobs.put(mo.getKey(), Math.min(mo.getValue() + 2, mobb));
            }
            else if (chr.hasEquipped(1142324)) {
            killedMobs.put(mo.getKey(), Math.min(mo.getValue() + 3, mobb));
            }
            else if (chr.hasEquipped(1142325)) {
            killedMobs.put(mo.getKey(), Math.min(mo.getValue() + 4, mobb));
            }
            else if (chr.hasEquipped(1142326)) {
