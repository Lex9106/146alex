...
public class Equip extends Item implements Serializable {
    ...
    private byte upgradeSlots = 0, level = 0, vicioushammer = 0, enhance = 0, enhanctBuff = 0, reqLevel = 0, yggdrasilWisdom = 0, karmaCount = -1;
    private short allStat = 0, reqJob = 0, imdR = 0, damR = 0,bdR = 0,hpR = 0, mpR = 0,str = 0, dex = 0, _int = 0, luk = 0, hp = 0, mp = 0, watk = 0, matk = 0, wdef = 0, mdef = 0, acc = 0, avoid = 0, hands = 0, speed = 0, jump = 0, charmExp = 0, pvpDamage = 0;
    ...
    public void renewPotential(int type, int line, int toLock) { // 0 = normal miracle cube, 1 = premium, 2 = epic pot scroll, 3 = super 
        final int rank = type == 2 ? -18 : type == 6 ? -20 : (Randomizer.nextInt(100) < 5 && getState() != (type == 3 ? 20 : 19) ? -(getState() + 1) : -(getState())); // 4 % chance to up 1 tier
        setPotential1(rank);
        if (getPotential3() > 0) {
            ...
            switch (type) {
                case 1: // premium-> suppose to be 25%
                    setPotential2(Randomizer.nextInt(10) <= 2.5 ? rank : 0); //1/10 chance of 3 line
                    break;
                case 2: // epic pot
               ...
               //     setBonusPotential2(Randomizer.nextInt(10) <= 2 ? rank : 0); //3/10 chance of 3 line
                    break;
                case 6: // This will be for Legendary Potential scroll
                    setPotential2(Randomizer.nextInt(10) <= 1 ? rank : 0); //2/10 chance of 3 line
                    break;
                default:
                    setPotential2(0);
        ...
        switch (line) {
            case 0:
                ...
                break;
            case 1:
                setPotential1(-(toLock + line * 100000));
    ...
    public void renewUniquePotential(int type, int line, int toLock) { // 0 = normal miracle cube, 1 = premium, 2 = epic pot scroll, 3 = super 
        final int rank = type == 2 ? -19 : (Randomizer.nextInt(100) < 5 && getState() != (type == 3 ? 20 : 19) ? -(getState() + 1) : -(getState())); // 4 % chance to up 1 tier
        setPotential1(rank);
        if (getPotential3() > 0) {
            setPotential2(rank); // put back old 3rd line
        } else {
    ...
    public void renewPotentialSPM(int type, int line, int toLock) { // 0 = normal miracle cube, 1 = premium, 2 = epic pot scroll, 3 = super 
        final int rank = type == 2 ? -18 : (Randomizer.nextInt(100) < 11 && getState() != (type == 3 ? 20 : 19) ? -(getState() + 1) : -(getState())); // 10% chance to up 1 tier
        setPotential1(rank);
        if (getPotential3() > 0) {
            setPotential2(rank); // put back old 3rd line
        } else {
                ...
                case 2: // epic pot
                    ...
                    break;
                case 3: // super
                    setPotential2(Randomizer.nextInt(10) < 0 ? rank : 0); //3/10 chance of 3 line
                    break;
                case 4: // revolutionary
                    setPotential2(Randomizer.nextInt(10) <= 3 ? rank : 0); //4/10 chance of 3 line
                    break;
    ...
    public void renewPotentialPMC(int type, int line, int toLock) { // 0 = normal miracle cube, 1 = premium, 2 = epic pot scroll, 3 = super 
        final int rank = type == 2 ? -18 : (Randomizer.nextInt(100) < 26 && getState() != (type == 3 ? 20 : 19) ? -(getState() + 1) : -(getState())); // 25% chance to up 1 tier
        setPotential1(rank);
        if (getPotential3() > 0) {
            setPotential2(rank); // put back old 3rd line
        } else {
    ...
    public static Equip calculateEquipStats(Equip eq) {
        ...
        if (eq.getTotalDamage() > 0) {
            ...
            System.out.println("Equip id " + eq.getItemId() + " has been added with Total Damage");
        }
        if (eq.getAllStat() > 0) {
