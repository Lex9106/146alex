...
public class DumpItems {
    ...
    public void dumpItem(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse, MapleData iz) throws Exception {
        ...
        if (dat != null) {
            ...
            for (final MapleData data : dat.getChildren()) {
                ...
                if (data.getName().startsWith("bdR")) {
                    final int gg = MapleDataTool.getIntConvert(data, 0);
                    if (gg != 0) {
                        rett.put(data.getName(), gg);
                    }
                System.out.println("Detected Stat bdR");
                }
                if (data.getName().startsWith("damR")) {
                    final int gg = MapleDataTool.getIntConvert(data, 0);
                    if (gg != 0) {
                        rett.put(data.getName(), gg);
                    }
                System.out.println("Detected Stat damR");
                }
                if (data.getName().startsWith("imdR")) {
                    final int gg = MapleDataTool.getIntConvert(data, 0);
                    if (gg != 0) {
                        rett.put(data.getName(), gg);
                    }
                System.out.println("Detected Stat imdR");
                }
                if (data.getName().startsWith("allStat")) {
                    final int gg = MapleDataTool.getIntConvert(data, 0);
                    if (gg != 0) {
                        rett.put(data.getName(), gg);
                    }
                System.out.println("Detected Stat allStat");
                }
            }
    ...
    protected final MapleData getStringData(final int itemId) {
        ...
        if (itemId >= 5010000) {
            ...
            data = eqpStringData;
            cat = "Eqp/Accessory";
        } else if (itemId >= 1172000 && itemId < 1202218) {
            data = eqpStringData;
            cat = "Eqp/MonsterBook";
            ...
            data = eqpStringData;
            cat = "Eqp/Glove";
        } else if (itemId >= 30000 && itemId < 47468) {
            data = eqpStringData;
            cat = "Eqp/Hair";
        }
        else if (itemId >= 1200000 && itemId < 1202218) {//bleh
            data = eqpStringData;
            cat = "Eqp/Totem";
        }
