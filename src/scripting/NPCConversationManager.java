...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public void getItemByName(String itemname) {        
        ...
        List <Integer> itemId = ii.getItemIdFromName(itemname, c.getPlayer());
        String text = null;
        if (itemId.size() > 0) {
        ...
        System.out.println("Item size is " + itemId.size() + " Id " + itemId);
        text += "#L" + itemId.get(i) + "##i" + itemId.get(i) + "# - " + "#z"+ itemId.get(i) + "#" + "\r\n";
        }   
       sendOk(text);   
    }
    public void getMobs(int itemid) {
       final List<Integer> mobs = MapleMonsterInformationProvider.getInstance().getMobByItem(itemid);
       System.out.println("Selection id is " + itemid);
       if (mobs.size() <= 0) {
        return;
       }
    ...
    public void sendNextPrevSNoESC(String text) {
        sendNextPrevS(text, (byte) 1, id);
    }

    public void sendNextPrevSNoESC(String text, int id) {
        sendNextPrevS(text, (byte) 1, id);
    }
    
     public void sendNextS(String text) {
        sendNextS(text, (byte) 3);
    }
    ...
    public void sendYesNoS(String text, byte type, int idd) {
        sendYesNoS(text, type, idd, id);
    }

    public void sendYesNoS(String text, byte type, int idd, int npcid) {
        if (lastMsg > -1) {
            return;
        }
        if (text.contains("#L")) { // will dc otherwise!
        ...
        c.getSession().write(NPCPacket.getNPCTalk(npcid, (byte) 2, text, "", type, idd));
        lastMsg = 2;
    }
...
//    }
    ...
    public void setHair(int hair) {
        if (hairExists(hair)) {
            ...
            //final String msg = ii.getName(hair);
            //FileoutputUtil.log("Hair.txt", "" + hair + ",//" + msg + "\r\n");
        }
    }
  ...
  public int gainGachaponItem(int id, int quantity, String msg, byte rareness) {
    try {
      ...
      if (rareness > 0) {
        World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(this.c.getPlayer().getName(), " : got a(n)", item, rareness, "Great Gachapierrot!"));
      }
      this.c.getSession().write(CWvsContext.InfoPacket.getShowItemGain(item.getItemId(), (short)quantity, true));
      return item.getItemId();
    } catch (Exception e) {
  ...
  public int gainCygnusItem(int id, int quantity) {
    return gainCygnusItem(id, quantity, this.c.getPlayer().getMap().getStreetName());
  }

  public int gainCygnusItem(int id, int quantity, String msg) {
    return gainCygnusItem(id, quantity, this.c.getPlayer().getMap().getStreetName(), (byte)0);
  }

  public int gainCygnusItem(int id, int quantity, String msg, byte rareness) {
    try {
      if (!MapleItemInformationProvider.getInstance().itemExists(id)) {
        return -1;
      }
      ...
      if (rareness > 0) {
        World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(this.c.getPlayer().getName(), " : got a(n)", item, rareness, "Cygnus Gachapon!"));
      }
      this.c.getSession().write(CWvsContext.InfoPacket.getShowItemGain(item.getItemId(), (short)quantity, true));
      return item.getItemId();
    } catch (Exception e) {
  ...
  public int gainMysteryScrollItem(int id, int quantity) {
    return gainMysteryScrollItem(id, quantity, this.c.getPlayer().getMap().getStreetName());
  }

  public int gainMysteryScrollItem(int id, int quantity, String msg) {
    return gainMysteryScrollItem(id, quantity, this.c.getPlayer().getMap().getStreetName(), (byte)0);
  }

  public int gainMysteryScrollItem(int id, int quantity, String msg, byte rareness) {
    try {
      if (!MapleItemInformationProvider.getInstance().itemExists(id)) {
        return -1;
      }
      ...
      if (rareness > 0) {
        World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(this.c.getPlayer().getName(), " : got a(n)", item, rareness, "Mystery Scroll!"));
      }
      this.c.getSession().write(CWvsContext.InfoPacket.getShowItemGain(item.getItemId(), (short)quantity, true));
      return item.getItemId();
    } catch (Exception e) {
  ...
  public int gainClawMachineItem(int id, int quantity) {
    return gainClawMachineItem(id, quantity, this.c.getPlayer().getMap().getStreetName());
  }

  public int gainClawMachineItem(int id, int quantity, String msg) {
    return gainClawMachineItem(id, quantity, this.c.getPlayer().getMap().getStreetName(), (byte)0);
  }

  public int gainClawMachineItem(int id, int quantity, String msg, byte rareness) {
    try {
      if (!MapleItemInformationProvider.getInstance().itemExists(id)) {
        return -1;
      }
      ...
      if (rareness > 0) {
        World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(this.c.getPlayer().getName(), " : got a(n)", item, rareness, "Claw Machine Gachapon!"));
      }
      this.c.getSession().write(CWvsContext.InfoPacket.getShowItemGain(item.getItemId(), (short)quantity, true));
      return item.getItemId();
    } catch (Exception e) {
    ...
    public int useNebuliteGachapon() {
        try {
            ...
            if (grade >= 3 && grade != 5) {
                World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(this.c.getPlayer().getName(), " : got a(n)", item, (byte) 2, "Nebulite Gachapon!"));
            }
    ...
    public int getWelcomeGiftLog(String bossid) {
        return getPlayer().getWelcomeGiftLog(bossid);
    }
    ...
    public void changeStat(byte slot, int type, int amount) {
        ...
        switch (type) {
            ...
            case 16:
                ...
                break;
            case 17:
                sel.setTotalDamage((byte) amount);
                break;
             case 18:
