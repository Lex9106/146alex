...
public abstract class AbstractPlayerInteraction {
    ...
    public final void ApplyBuff(final int id) {
        SkillFactory.getSkill(id).getEffect(1).applyTo(c.getPlayer());
    }

    public final void RemoveBuff(final int id) {
        c.getPlayer().dispelBuff(id);
    }
...
//    }
    ...
    public final void gainItem(final int id, final short quantity, final boolean randomStats, final long period, boolean hours, final int slots, final String owner, final MapleClient cg, final boolean show) {
        if (quantity >= 0) {
            ...
            if (type.equals(MapleInventoryType.EQUIP) && !GameConstants.isThrowingStar(id) && !GameConstants.isBullet(id)) {
                final Equip item = (Equip) (randomStats ? ii.randomizeStats((Equip) ii.getEquipById(id), null, null) : ii.getEquipById(id));
                if (period > 0) {
    ...
    public void gainQuestExp(int gain) {
        c.getPlayer().gainExp(gain * GameConstants.getExpRate_Quest(c.getPlayer().getStat()) * (c.getPlayer().getStat().questBonus) / 100 + gain, true, true, true);
    }
    
    public void gainHonor(int gain) {
        c.getPlayer().addHonourExp(gain);
    }
